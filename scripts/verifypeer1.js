...
async function main() {
    const { peer1 } = await getNamedAccounts();
    const Main = await ethers.getContract("Main", peer1);
    console.log(`Got contract Main at ${Main.address}`);
    console.log("peer1 verifying file...");
    const version = await Main.checkVersion();
    // const PEER1_VERSION = fs.readFileSync("peer1/peer1version.txt").tostring();
    const PEER1_VERSION = ""

    if (PEER1_VERSION != version) {
        ...
        //     });
        const transactionResponse = await Main.verify(true, peer1);
